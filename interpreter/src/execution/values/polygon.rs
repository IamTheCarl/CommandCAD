...
impl Object for PolygonSet {
    ...
    fn get_attribute(
        ...
        match attribute {
            "iter_polys" => {
                Ok(BuiltinFunction::new::<methods_and_functions::polygon_set::IterPolys>().into())
            }
            "num_poly" => {
...
#[derive(Debug, Eq, PartialEq, Clone)]
pub struct PolygonSetIterator {
    polygon_set: PolygonSet,
}

impl IterableObject for PolygonSetIterator {
    fn iterate<R>(
        &self,
        _context: &ExecutionContext,
        callback: impl FnOnce(&mut dyn Iterator<Item = ExecutionResult<Value>>) -> ExecutionResult<R>,
        ...
        let mut iter = self
            ...
            .0
            .iter()
            .map(|polygon| Polygon(Arc::new(polygon.clone())).into())
            .map(Ok);
        callback(&mut iter)
    }
}
...
pub mod methods_and_functions {
    ...
    pub mod polygon {
        ...
        impl CircleInfo {
            fn new(
                ...
                distance_between_points: Option<Length>,
                number_of_points: Option<UnsignedInteger>,
            ) -> ExecutionResult<Self> {
                const SEGMENT_ANGLE_RADIANS: RawFloat = 2.0;
        ...
        pub fn register_methods_and_functions(database: &mut BuiltinCallableDatabase) {
            ...
            build_function!(
                ...
                Circle, "Polygon::circle", (
                    ...
                    distance_between_points: Option<Length> = ValueNone.into(),
                    number_of_points: Option<UnsignedInteger> = ValueNone.into()) -> Polygon
                {
                    ...
                    let mut points = Vec::with_capacity(circle_info.number_of_points);
                    for point_index in 0..circle_info.number_of_points {
                        let angle_to_point = circle_info.segment_angle * point_index as RawFloat * PI;
                        let (y, x) = angle_to_point.sin_cos();
                        let coord = geo::Coord { x: x * circle_info.radius, y: y * circle_info.radius };
                        points.push(coord);
                    }

                    Ok(Polygon(Arc::new(geo::Polygon::new(geo::LineString(points), vec![]))))
                }
            );
            build_function!(
                database,
            ...
            build_method!(
                ...
                {
                    let polygon = Arc::unwrap_or_clone(this.0);
    ...
    pub mod polygon_set {
        use crate::values::polygon::{ApplyTransform as _, PolygonSetIterator};

        use super::*;

        pub struct FromPolys;
        pub struct IterPolys;
        pub struct NumPoly;
        pub struct Area;
        pub struct BoundingBox;
        pub struct Centroid;
        ...
        pub fn register_methods_and_functions(database: &mut BuiltinCallableDatabase) {
            build_function!(
                ...
                {
                    let mut collected = Vec::with_capacity(polys.len());
                    for value in polys {
                        let polygon: Polygon = value.downcast(context)?;
                        let polygon = Arc::unwrap_or_clone(polygon.0);
                        collected.push(polygon);
                    }

                    Ok(PolygonSet(Arc::new(geo::MultiPolygon(collected))))
                }
            );

            build_method!(
                database,
                IterPolys, "PolygonSet::iter_polys", (
                    context: &ExecutionContext,
                    this: PolygonSet) -> ValueIterator
                {
                    Ok(ValueIterator::new(PolygonSetIterator { polygon_set: this }))
                }
            );
            build_method!(
                database,
            ...
            build_method!(
                ...
                BoundingBox, "PolygonSet::bounding_box", (
                    context: &ExecutionContext,
                    this: PolygonSet) -> Option<Dictionary>
                {
                    bounding_box(context, &*this.0)
                }
            );
            build_method!(
                ...
                Centroid, "PolygonSet::centroid", (
                    context: &ExecutionContext,
                    this: PolygonSet) -> Option<Vector2>
                {
                    centroid(context, &*this.0)
                }
            );
...
mod test {
    ...
    use geo::{Coord, Distance as _, Euclidean, Point};
    ...
    fn polygon_from_circle() {
        ...
        for (index, (actual, expected)) in expected_coords
            .iter()
            .zip(polygon.0.exterior().0.iter())
            .enumerate()
        {
            assert!(
                Euclidean.distance(Point(*actual), Point(*expected)) < 0.00001,
                "Coordinate {index} was not within tolerance"
            );
        }

        let result =
            test_run("std.polygon.circle(diameter = 1m, angle_between_points = 0.5rad)").unwrap();
        let polygon = result.as_polygon().unwrap();
        let expected_coords = [
            Coord { x: 0.5, y: 0.0 },
            Coord { x: 0.0, y: 0.5 },
    ...
    fn polygon_into_set() {
        let result = test_run(
            "std.polygon.from_points(points = [{0m, 0m}, {1m, 0m}, {1m, 1m}, {0m, 1m}])::into_set()",
        )
        .unwrap();
        assert_eq!(
            result,
            PolygonSet(Arc::new(geo::MultiPolygon(vec![geo::Polygon::new(
                geo::LineString(vec![
                    Coord { x: 0.0, y: 0.0 },
                    Coord { x: 1.0, y: 0.0 },
                    Coord { x: 1.0, y: 1.0 },
    ...
    #[test]
    fn polygon_set_from_polys() {
        let result = test_run(
            "std.polygon_set.from_polys(polys = [std.polygon.from_points(points = [{0m, 0m}, {1m, 0m}, {1m, 1m}, {0m, 1m}]), std.polygon.from_points(points = [{1m, 1m}, {2m, 1m}, {2m, 2m}, {1m, 2m}])])",
        )
        .unwrap();
        assert_eq!(
            result,
            PolygonSet(Arc::new(geo::MultiPolygon(vec![
                geo::Polygon::new(
                    geo::LineString(vec![
                        Coord { x: 0.0, y: 0.0 },
                        Coord { x: 1.0, y: 0.0 },
    ...
    #[test]
    fn polygon_set_iter_polys() {
        run_assert_eq(
            "std.polygon_set.from_polys(polys = [std.polygon.from_points(points = [{0m, 0m}, {1m, 0m}, {1m, 1m}, {0m, 1m}]), std.polygon.from_points(points = [{1m, 1m}, {2m, 1m}, {2m, 2m}, {1m, 2m}])])::iter_polys()::collect_list()",
            "[std.polygon.from_points(points = [{0m, 0m}, {1m, 0m}, {1m, 1m}, {0m, 1m}]), std.polygon.from_points(points = [{1m, 1m}, {2m, 1m}, {2m, 2m}, {1m, 2m}])]",
        );
    }

    #[test]
    fn polygon_set_num_poly() {
        run_assert_eq(
            "std.polygon_set.from_polys(polys = [std.polygon.from_points(points = [{0m, 0m}, {1m, 0m}, {1m, 1m}, {0m, 1m}]), std.polygon.from_points(points = [{1m, 1m}, {2m, 1m}, {2m, 2m}, {1m, 2m}])])::num_poly()",
            "2u",
        );
    }
    ...
    fn polygon_set_area() {
        run_assert_eq(
            "std.polygon.from_points(points = [{0m, 0m}, {1m, 0m}, {1m, 1m}, {0m, 1m}])::into_set()::area()",
            "1 'm^2'",
        );
    }

    #[test]
    fn polygon_set_bounding_box() {
        run_assert_eq(
            "std.polygon.from_points(points = [{0m, 0m}, {1m, 0m}, {1m, 1m}, {0m, 1m}])::into_set()::bounding_box()",
            "(min = {0m, 0m}, max = {1m, 1m})",
        );
    }

    #[test]
    fn polygon_set_centroid() {
        run_assert_eq(
            "std.polygon.from_points(points = [{0m, 0m}, {1m, 0m}, {1m, 1m}, {0m, 1m}])::into_set()::centroid()",
            "{0.5m, 0.5m}",
        );
    }

    #[test]
    fn polygon_set_transform() {
        run_assert_eq(
            "std.polygon.from_points(points = [{0m, 0m}, {1m, 0m}, {1m, 1m}, {0m, 1m}])::into_set()::transform(t = std.consts.Transform2d::translate(offset = {1m, 2m}))",
            "std.polygon.from_points(points = [{1m, 2m}, {2m, 2m}, {2m, 3m}, {1m, 3m}, {1m, 2m}])::into_set()",
        );
    }
}
